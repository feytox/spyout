...
public class GridController : MonoBehaviour
{
    ...
    private void Awake()
    {
        Debug.Assert(
            _singleton == null,
            $"{gameObject.name} tried to awake {nameof(GridController)} second time."
        );
        _singleton = this;
    ...
    public static IEnumerable<Vector3Int> FindPath(
        GameObject walker,
        Vector3 startPos,
        Vector3 endPos
    )
    ...
    private static GridController GetInstance()
    {
        Debug.Assert(
            _singleton != null,
            $"Tried to access {nameof(GridController)} before it was initialized!"
        );
        return _singleton;
    }
}
